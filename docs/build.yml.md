...
| Compile Linux binary (debug) | Runs `make debug` inside the same container and volume layout, producing `build/studystudio-0.0.12_linux_debug` (see [Makefile](Makefile.md)). |
...
- There is no Windows debug job, because the `Makefile` has no `win64-debug` target — only Linux debug builds exist (see [Makefile current limitations](Makefile.md#current-limitations)).
