...
- Neither debug step attaches a debugger — they only compile the `-g -O0 -DDEBUG` binaries and upload them as artifacts. This workflow has no GDB/`winedbg` step for either platform (see [Makefile](Makefile.md#current-limitations)).
